# Fastfetch
## tutorial to remove x - □ and other things
to remove the x - □ and other things, scroll down for a tutorial

## Instalation
To install Fastfetch watch this [vidio](https://youtu.be/z3NpVq-y6jU?si=8u3w14umI5Dfv2sd)

## Config
The Fastfetch config is from here: [fastfetch-Config](https://github.com/AziGoss/Dotfiles/fastfetch)

## My Config
But if u want my config then just download [my-config](https://github.com/Shadowniko1/My-Files/blob/main/Fastfetch/config.jsonc)
<img width="1331" height="690" alt="image" src="https://github.com/user-attachments/assets/67344bed-d0e2-49bb-b672-8939e9ccd915" />

## Tutorial to remove the - □ x and other things
1. go to settings> startup
2. make the launch size 120 x 30
3. make the Launch parameters like this: Focus, let windows decide, centered
4. and if u have komrebi and want to force it to not recise it than copy my [config](https://github.com/Shadowniko1/My-Files/blob/main/Fastfetch/config.jsonc)
